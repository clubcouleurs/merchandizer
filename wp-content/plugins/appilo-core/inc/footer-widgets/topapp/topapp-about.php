<?php
// Adds widget: Topapp About
class Topappabout_Widget extends WP_Widget {

    function __construct() {
        parent::__construct(
            'topappabout_widget',
            esc_html__( 'Topapp About', 'appilo' ),
            array( 'description' => esc_html__( 'Add topapp about widget', 'appilo' ), ) // Args
        );
    ...
    private $widget_fields = array(
        array(
            'label' => 'Upload Logo',
            'id' => 'topapp_widget_logo',
            'type' => 'media',
        ),
        array(
            'label' => 'About Text',
            'id' => 'topapp_widget_about_text',
            'type' => 'textarea',
        ),
        array(
            'label' => 'Payment Logo',
            'id' => 'topapp_widget_payment_logo',
            'type' => 'media',
        ),
        array(
            'label' => 'Payment Logo URL',
            'id' => 'topapp_widget_payment_logo_url',
            'type' => 'text',
        ),
    );

    public function widget( $args, $instance ) {
        $widget_logo_id = apply_filters( 'footer_logo_image_uri', $instance[ 'topapp_widget_logo' ] );
        $widget_logo= wp_get_attachment_url($widget_logo_id);
        $widget_payment_logo_id = apply_filters( 'footer_payment_image_uri', $instance[ 'topapp_widget_payment_logo' ] );
        $widget_payment_logo= wp_get_attachment_url( $widget_payment_logo_id );
        echo $args['before_widget'];
        // Output generated fields
        echo '<div class="footer-widget logo-widget">
                        <div class="logo">
                            <a href="'.esc_url(home_url('/')).'"><img src="'.esc_url($widget_logo).'" alt="Footer Logo"/></a>
                        </div>
                        <div class="text">'.$instance['topapp_widget_about_text'].'</div>
                        <div class="paypall">
                            <a href="'.esc_url($instance['topapp_widget_payment_logo_url']).'"><img src="'.esc_url($widget_payment_logo).'" alt="Payment Logo"/></a>
                        </div>
                    </div>';
...
function register_topappabout_widget() {
    register_widget( 'Topappabout_Widget' );
}
add_action( 'widgets_init', 'register_topappabout_widget' );